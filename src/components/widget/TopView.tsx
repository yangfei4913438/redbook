...
export default class TopView extends Component<any, any> {
  ...
  componentDidMount() {
    ...
    DeviceEventEmitter.addListener('removeAllOverlay-Alert', () => this.removeAll());
  ...
  render() {
    ...
    if (laodingItem) {
      return (
        <View style={{ flex: 1 }}>
          <Animated.View style={{ flex: 1, transform: transform }}>{this.props.children}</Animated.View>
          ...
          <View style={styles.overlayContainer} pointerEvents={'box-none'}>
            ...
            {toastElements.map((item, index) => {
              ...
              if (index === toastElements.length - 1) {
                return (
                  <View key={'TopView_Toast' + item.key} style={styles.overlay} pointerEvents={'box-none'}>
                    {item.element}
                  </View>
                );
              } else {
